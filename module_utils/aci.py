...
# Collector function must return a dictionary with keys elastic_docs and debug_msgs
...
def aci(config_data):
    ...
    for fabric_name in config_data['apics'].keys():
        ...
        if login_result['rc'] != 0:
            for error in login_result['error']:
                msgs.append(error)
                return {'elastic_docs': elastic_docs, 'debug_msgs': msgs}
    ...
    return {'elastic_docs': elastic_docs, 'debug_msgs': []}
